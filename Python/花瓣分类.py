...
x, y = np.split(data,                                       #要切分的数组
                (4,),                                       #沿轴切分的位置，第5列开始往后为y
                axis=1)                                     #代表纵向分割，按列分割
x = x[:, 2:4]                                               #在X中我们取第3、4列作为特征（花瓣长度和花瓣宽度），为了后面的可视化。x[:,2:4]代表第一维(行)全取，第二维(列)取2~4
print(x)
...
def classifier():
    ...
    clf = svm.SVC(C=0.5,                         #误差项惩罚系数,默认值是1
                  kernel='linear',               #线性核 kenrel="rbf":高斯核   linear
...
def draw(clf, x):
    ...
    plt.xlabel(iris_feature[2], fontsize=20)  # 花瓣长度
    plt.ylabel(iris_feature[3], fontsize=20)  # 花瓣宽度
