...
//
//    【为什么加起来就能满足所有方程？—— 模运算的加法分配律】
//    
//    核心性质：(a + b) mod m = ((a mod m) + (b mod m)) mod m
//    
//    验证 x = x₁ + x₂ + ... + xₙ 满足第 i 个方程：
//    
//    x mod m[i] = (x₁ + x₂ + ... + xₙ) mod m[i]
//                = (x₁ mod m[i] + x₂ mod m[i] + ... + xₙ mod m[i]) mod m[i]  ← 加法分配律！
//                = (0 + 0 + ... + a[i] + ... + 0) mod m[i]  ← xⱼ ≡ 0 (mod m[i]) 当 j≠i
//                = a[i] ✓
//    
//    本质：模运算下加法可以"穿透"到每一项，而我们精心构造了每一项：
//    - xᵢ 对自己的方程贡献 a[i]（"开关打开"）
//    - xᵢ 对其他方程贡献 0（"开关关闭"）
//    加法把所有"开关"的贡献叠加起来，每个方程恰好只看到自己的那一项！
//    
//    【为什么解相差 M 的倍数？核心证明】
//    
//    假设 x₀ 是一个解，即：
