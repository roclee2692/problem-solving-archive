...
ll occ[N];       // 访问计数：occ[u] = 在文本匹配时到达节点 u 的次数
                 // 用于统计每个节点被访问的频率
...
inline void buildfail() {
    ...
    for (ll c = 0; c < L; c++) {
        ...
        if (v) {
            ...
            // 如果子节点不存在，让 nxt[root][c] 指向根（自环）
            nxt[root][c] = root;
            // 这样匹配时从根读取任意不存在的字符，都会回到根
        }
    }
...
inline ll matchCountDistinct(const string& text) {
    ...
    for (int i = (int)bfsorder.size() - 1; i >= 0; --i) {
        ...
        // 【为什么这样做？】
        // fail[x] 是 x 的最长真后缀
        // 如果字符串 str(x) 出现了，那么 str(fail[x]) 也一定出现
        // 例如：x = "abc"，fail[x] = "bc"
        //       如果 "abc" 出现 2 次，则 "bc" 至少出现 2 次
        
        // 【为什么逆序？】
        // bfsorder 是 BFS 顺序（深度递增）
        // 逆序遍历确保先处理子节点，再处理父节点
        // 这样才能正确累加贡献
    }
